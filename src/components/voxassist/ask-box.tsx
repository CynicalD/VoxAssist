...
/**
 * Prompt input for VoxAssist. Builds a {@link Scope} from a self/other-user
 * toggle plus a target username, and emits either an `ask` or `plan` request.
 *
 ...
 */
export function AskBox({ currentUser, onAsk, onPlan, busy = false }: AskBoxProps) {
  ...
  function handleKeyDown(e: React.KeyboardEvent<HTMLTextAreaElement>) {
    // Enter submits; Shift+Enter for newline. Respect IME composition.
    ...
    >
      ...
      {friendMode && (
        <p className="mt-2 text-xs text-muted-foreground">
          Only notes {targetUser.trim() ? <span className="font-medium">@{targetUser.trim()}</span> : "the target user"} marked
          shared will be searched.
        </p>
      )}
    </form>
  )
